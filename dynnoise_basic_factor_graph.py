...
def main(args):
    ...
    for i in range(N-1):
        ...
        odometry_factor = gtsam.BetweenFactorPose2(i, i+1, odometry_meas[i], ODOMETRY_NOISE)
    ...
    # Optimize
    optimized_estimate = optimizer.optimize() # Estimated pose of the robot at w.r.t. the world frame; H_i_to_w
    ...
    if use_GPS and do_loopclosure:
        plt.title("Noise, Odometry and GPS, Loop-Closure")
    elif use_GPS:
        plt.title("Noise, Odometry and GPS")
    elif do_loopclosure:
        plt.title("Noise, Odometry, Loop-Closure")
    else:
        plt.title("Noise, only Odometry")
    plt.show()
